...
export async function query(text: string, params?: any[]) {
  if (isPostgres && prisma) {
    try {
      // Prisma's $queryRawUnsafe allows direct running of SQL text with arguments
      const res = await prisma.$queryRawUnsafe<any[]>(text, ...(params || []));
      return res;
    } catch (err) {
    ...
    // SQLite uses '?' placeholder, so we replace $1, $2... with ?
...
export async function initDb() {
  const sqliteSchema = `
    CREATE TABLE IF NOT EXISTS admins (id TEXT PRIMARY KEY, firstName TEXT, lastName TEXT, passwordHash TEXT);
    CREATE TABLE IF NOT EXISTS teachers (id TEXT PRIMARY KEY, firstName TEXT, lastName TEXT, classIds TEXT, passwordHash TEXT);
    CREATE TABLE IF NOT EXISTS students (id TEXT PRIMARY KEY, firstName TEXT, lastName TEXT, classId TEXT, className TEXT, nationalId TEXT, dateOfBirth TEXT, birthCert TEXT, placeOfBirth TEXT, nationality TEXT, dominantHand TEXT, health TEXT, family TEXT, contact TEXT, naseebData TEXT, profilePictureUrl TEXT, passwordHash TEXT);
  ...
  // For PostgreSQL, let's execute raw tables creation if Prisma did not handle it
  const pgSchema = `
    CREATE TABLE IF NOT EXISTS admins (id VARCHAR(255) PRIMARY KEY, firstName VARCHAR(255), lastName VARCHAR(255), "passwordHash" VARCHAR(255));
    CREATE TABLE IF NOT EXISTS teachers (id VARCHAR(255) PRIMARY KEY, firstName VARCHAR(255), lastName VARCHAR(255), "classIds" JSONB, "passwordHash" VARCHAR(255));
    CREATE TABLE IF NOT EXISTS students (id VARCHAR(255) PRIMARY KEY, firstName VARCHAR(255), lastName VARCHAR(255), "classId" VARCHAR(255), "className" VARCHAR(255), "nationalId" VARCHAR(50), "dateOfBirth" VARCHAR(50), "birthCert" JSONB, "placeOfBirth" VARCHAR(255), nationality VARCHAR(100), "dominantHand" VARCHAR(50), health JSONB, family JSONB, contact JSONB, "naseebData" JSONB, "profilePictureUrl" TEXT, "passwordHash" VARCHAR(255));
  ...
  if (isPostgres && prisma) {
    try {
      ...
      const checkSettings = await prisma.$queryRawUnsafe<{ count: string }[]>(
        'SELECT count(*) as count FROM settings WHERE id = 1'
      );
      const count = parseInt((checkSettings[0] as any).count || '0');
      if (count === 0) {
        await prisma.$executeRawUnsafe("INSERT INTO settings (id, data) VALUES (1, '{}')");
      }
    } catch (err) {
...
export const db = isPostgres ? prisma : sqliteDb;
export { isPostgres, prisma };
