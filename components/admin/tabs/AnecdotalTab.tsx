...
import type { Admin, AnecdotalRecord, Student, SchoolClass } from '../../../types';
...
const AnecdotalRecordModal: React.FC<AnecdotalRecordModalProps> = ({ recordToEdit, students, classes, subjects, onClose, onSubmit, adminId, years }) => {
    ...
    const studentsToDisplay = useMemo(() => {
        ...
        return students
            .filter(s => classFilter.includes(s.classId))
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4" onClick={onClose}>
            <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-3xl max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
                ...
                <form onSubmit={handleSubmit} className="flex-grow flex flex-col space-y-4 min-h-0">
                    ...
                    <div className="flex-grow grid grid-cols-1 md:grid-cols-2 gap-4 min-h-0 border-t pt-4">
                        <div>
                            <label className="font-semibold text-sm">فیلتر کلاس</label>
                            <div className="border rounded-md p-2 h-48 overflow-y-auto mt-1">
                                {classes.map(c => <label key={c.id} className="flex items-center gap-2"><input type="checkbox" checked={classFilter.includes(c.id)} onChange={() => setClassFilter(p => p.includes(c.id) ? p.filter(id => id !== c.id) : [...p, c.id])} />{c.name}</label>)}
                            </div>
                        </div>
                        <div>
                            <label className="font-semibold text-sm">دانش آموز(ان)</label>
                            <div className="border rounded-md p-2 h-48 overflow-y-auto mt-1">
                                {studentsToDisplay.map(s => <label key={s.id} className="flex items-center gap-2"><input type="checkbox" checked={studentIds.includes(s.id)} onChange={() => setStudentIds(p => p.includes(s.id) ? p.filter(id => id !== s.id) : [...p, s.id])} />{formatFullName(s)}</label>)}
                            </div>
                        </div>
                    </div>
                    <div className="flex justify-end gap-4 pt-4 border-t"><button type="button" onClick={onClose}>انصراف</button><button type="submit">{recordToEdit ? 'ذخیره' : 'ثبت'}</button></div>
                </form>
            </div>
        </div>
    );
...
const AnecdotalTab: React.FC<AnecdotalTabProps> = ({ admin, years }) => {
    const { students, classes, teachers, anecdotalRecords, saveAnecdotalRecord, deleteAnecdotalRecord } = useData();
    ...
    const enrichedRecords = useMemo(() => {
        return anecdotalRecords.map(rec => ({
            ...rec,
            studentNames: rec.studentIds.map(id => formatFullName(students.find(s => s.id === id)) || 'حذف شده').join('، '),
            recordedByName: formatFullName(allRecorders.find(r => r.id === rec.recordedBy)) || 'سیستم',
        }));
    }, [anecdotalRecords, students, allRecorders]);

    const { items: sortedRecords, requestSort, sortConfig } = useSortableData(enrichedRecords, [{key: 'date', direction: 'descending'}]);

    return (
        <div className="space-y-4">
            <div className="flex justify-between items-center"><h2 className="text-xl font-bold">واقعه نگاری</h2><button onClick={() => {setRecordToEdit(null); setModalOpen(true);}} className="px-4 py-2 bg-blue-500 text-white rounded-md">ثبت واقعه جدید</button></div>
            ...
            {modalOpen && <AnecdotalRecordModal recordToEdit={recordToEdit} students={students} classes={classes} subjects={allSubjects} onClose={() => setModalOpen(false)} onSubmit={rec => {saveAnecdotalRecord(rec); setModalOpen(false);}} adminId={admin.id} years={years} />}
        </div>
    );
};
